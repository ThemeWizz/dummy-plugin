...
if (!class_exists('WP_Package_Updater')) {
	...
	class WP_Package_Updater
	{
		...
		public function __construct($package_file_path, $package_path)
		{
			...
			if ('Plugin' === $this->type) {
				$package_slug = $package_file_path_parts[count($package_file_path_parts) - 2];
			...
			$this->update_server_url = trailingslashit($update_server_url) . 'twz-us-update-api/';
			...
			if ($this->use_license) {
				$this->license_server_url = trailingslashit($update_server_url) . 'twz-us-license-api/';
		...
		public function filter_update_checks($query_args)
		{
			...
			if ($this->use_license) {
				$license           = get_option($this->package_slug . '_licenseKey');
				$license_signature = get_option($this->package_slug . '_licenseSignature');
		...
		public function activate_license()
		{
			...
			if (isset($license_data->package_slug, $license_data->license_key)) {
				update_option($this->package_slug . '_licenseKey', $license_data->license_key);

				if (isset($license_data->license_signature)) {
					update_option($this->package_slug . '_licenseSignature', $license_data->license_signature);
				} else {
					delete_option($this->package_slug . '_licenseSignature');
				}
			} else {
				$error = new WP_Error($this->package_slug . '_License', $license_data->message);

				if (property_exists($license_data, 'clear_key') && $license_data->clear_key) {
					delete_option($this->package_slug . '_licenseSignature');
					delete_option($this->package_slug . '_licenseKey');
				}

				wp_send_json_error($error);
			}

			$this->delete_option($this->package_slug . '_licenseError');
		...
		public function deactivate_license()
		{
			...
			if (isset($license_data->package_slug, $license_data->license_key)) {
				update_option($this->package_slug . '_licenseKey', '');

				if (isset($license_data->license_signature)) {
					update_option($this->package_slug . '_licenseSignature', '');
				} else {
					delete_option($this->package_slug . '_licenseSignature');
				}
			} else {
		...
		protected function handle_license_errors($license_data, $query_type = null)
		{
			...
			if (isset($license_data->status) && 'expired' === $license_data->status) {
				...
				if (isset($license_data->date_expiry)) {
					...
					$date->setTimestamp(strtotime($license_data->date_expiry));
			...
			} elseif (isset($license_data->status) && 'on-hold' === $license_data->status) {
				$license_data->message   = __('The license is on hold and cannot be updated. Please use another license key or contact to customer support.', 'wp-package-updater');
			} elseif (isset($license_data->status) && 'deactivated' === $license_data->status) {
				$license_data->message   = __('The license is deactivated and cannot be updated. Please use another license key or contact to customer support.', 'wp-package-updater');
